...
function createRecipesStore() {
	...
	const loadRecipes = async () => {
		...
		// Save a new recipe
		saveRecipe: async (title: string, description: string, ingredients: Ingredient[], isPublic: boolean = false) => {
			update(state => ({ ...state, loading: true, error: null }));
		...
		updateRecipe: async (recipeId: string, updates: { title?: string; description?: string; ingredients?: Ingredient[]; isPublic?: boolean }) => {
			update(state => ({ ...state, loading: true, error: null }));
